...
longest (x:xs) = longest' xs x
  where
    longest' [] l = l
    longest' (x:xs) l
      | length x > length l = longest' xs x
      | length x == length l && head x < head l = longest' xs x
      | otherwise = longest' xs l
...
incrementKey k [] = []
incrementKey k (x:xs)
  | k == fst x = (key, value + 1) : incrementKey k xs
...
transfer from to amount bank
  | Map.notMember from bank == True = bank
  | Map.notMember to bank == True = bank
  | amount < 0 = bank
  | availableFunds == 0 = bank
  | availableFunds - amount < 0 = bank
  | otherwise = Map.alter deposit to $ Map.alter withdraw from bank
  where
    availableFunds = Map.findWithDefault 0 from bank 
    withdraw (Just x) = Just (x - amount)
    deposit (Just x) = Just (x + amount)
...
maxIndex arr = getMax arrListTail arrListHead
  where
    getMax [] i = fst i
    getMax (x:xs) i = if snd x > snd i then getMax xs x else getMax xs i 
    arrList = assocs arr
    arrListHead = head arrList
    arrListTail = tail arrList
