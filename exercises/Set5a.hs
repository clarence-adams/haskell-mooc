...
buyOneMore (MkShoppingEntry str price count) = MkShoppingEntry str price (count + 1)
...
setName name p = Person { name = name, age = getAge p }

-- setAge does likewise for age
setAge :: Int -> Person -> Person
setAge age p = Person { name = getName p, age = age }
...
rgb Green = [0, 1, 0]
rgb Blue  = [0, 0, 1]
rgb (Mix c1 c2) = [avgR, avgG, avgB]
  where
    firstColor = rgb c1
    secondColor = rgb c2
    avgR = (firstColor!!0 + secondColor!!0) / 2
    avgG = (firstColor!!1 + secondColor!!1) / 2
    avgB = (firstColor!!2 + secondColor!!2) / 2
rgb (Invert c1) = [invR, invG, invB]
  where
    firstColor = rgb c1
    invR = 1 - firstColor!!0
    invG = 1 - firstColor!!1
...
toNat i
  | i < 0 = Nothing
  | i == 0 = Just Zero
  | otherwise = Just (PlusOne (go (i - 1)))
    where
      go i
        | i == 0 = Zero
        | otherwise = PlusOne (go (i - 1))
...
getBinaryList :: Bin -> [Int]
getBinaryList End = []
getBinaryList (O b) = getBinaryList b ++ [0]
getBinaryList (I b) = getBinaryList b ++ [1]

fromBin :: Bin -> Int
fromBin b = snd (foldr convertToInt (0, 0) binaryList)
  where
    convertToInt x (y, yy)
      | x == 0 = (y + 1, yy)
      | otherwise = (y + 1, yy + 2^y)
    binaryList = getBinaryList b

toBin :: Int -> Bin
toBin 0 = (O End)
toBin i = inc (toBin (i - 1))
