...
whileRight check x = checked (check x)
  where
    checked (Left x) = x
    checked (Right x) = whileRight check x
...
joinToLength i strings = [ s ++ s2  | s <- strings, s2 <- strings, length (s ++ s2) == i ]
...
sumRights xs = sumRights' xs 0
  where
    sumRights' [] a = a
    sumRights' ((Left x):xs) a = sumRights' xs a
    sumRights' ((Right x):xs) a = sumRights' xs a + x
