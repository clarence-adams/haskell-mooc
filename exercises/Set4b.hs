...
countHelper Nothing ys = 1 + ys
countHelper (Just x) ys = 0 + ys
...
maxHelper y ys = if y > ys then y else ys
...
slStart = (0.0, 0)
slHelper x xs = (x + fst xs, 1 + snd xs)
...
largestHelper y ys = filter (\e -> e == maximum (y:ys)) (y:ys)
